...
const PaymentActions = ({ order, verifyingPayment, onSendToKitchen, onFinalizePayment, onVerifyPayment, orderIsSent, paymentVerified }) => {
  ...
  return (
    <div className="p-4 mt-4 bg-white rounded-lg shadow-sm">
      ...
      <div className="mt-2 text-xs text-gray-500">
        Debug: orderIsSent={String(orderIsSent)}, order.status={order?.status}
      </div>

      {order && order.status === 'pending' && !paymentVerified ? (
